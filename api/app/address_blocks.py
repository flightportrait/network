"""State of registration from the 24-bit ICAO aircraft address.

ICAO Annex 10 Volume III (appendix to chapter 9) allocates address
blocks to states. The state is a pure function of the hex: computed at
serve time, never stored, no source to license. Checked against the
registration prefixes of the tar1090-db registry on 2026-09-23: every
block whose top prefix covered under 80 % of its aircraft was a state
with several prefixes (Brazil PP/PR/PS/PT) or military serials.

Some blocks sit inside others (Bermuda and the Cayman Islands inside the
United Kingdom, Hong Kong inside China); the smallest block wins. ICAO
temporary and special-use blocks have no state and return None.
"""
# (first, last, ISO 3166-1 alpha-2), sorted by first address.
BLOCKS = (
...
def state_of(hex_id):
    ...
    The smallest containing block wins; 200 blocks, a scan is enough."""
    try:
        address = int(hex_id, 16)
    ...
    for first, last, state in BLOCKS:
        if first > address:
            break
        if address <= last and (best is None
                                or last - first < best[1] - best[0]):
            best = (first, last, state)
